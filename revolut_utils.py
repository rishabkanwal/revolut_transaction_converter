...
def fetch_timeframe_quotes(
    ...
    def chunked_timeframes() -> Iterable[tuple[date, date]]:
        chunk_start = start_date
        max_delta = timedelta(days=MAX_TIMEFRAME_DAYS - 1)
        while chunk_start <= end_date:
            chunk_end = min(chunk_start + max_delta, end_date)
            yield chunk_start, chunk_end
            chunk_start = chunk_end + timedelta(days=1)

    combined_quotes: dict[str, Mapping[str, float]] = {}
    source_currency: str | None = None

    for chunk_start, chunk_end in chunked_timeframes():
        params = {
        ...
        chunk_source = data.get("source", "USD")
        if source_currency and chunk_source != source_currency:
            raise RuntimeError(
                "Exchange rate API returned inconsistent source currencies "
                f"({source_currency} and {chunk_source})."
            )
        source_currency = chunk_source
        combined_quotes.update(quotes)

    return source_currency or "USD", combined_quotes
